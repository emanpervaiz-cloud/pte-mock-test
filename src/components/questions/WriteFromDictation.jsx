...
const WriteFromDictation = ({ question, onNext }) => {
  ...
  const handleGetScore = async () => {
    if (!sentence.trim() || evalLoading) return;
    
    setEvalLoading(true);
    setEvalError(null);
    
    try {
      const result = await aiService.evaluateListeningQuestion(question, sentence.trim());
  ...
  const handleSubmit = () => {
    ...
    saveAnswer(question.id, {
      ...
      section: 'listening',
      correct_answer: question.correctResponse || question.transcript,
      response: sentence,
